...
export function validation (userData){
    ...
    if(!userData.username) errors.username ="user cannot be emty";
    if(!userData.username.length >35) errors.username = "Username cannot exceed 35 char";

    if(!regexPassword.test(userData.password)) errors.password = "Password must have al least one number";
